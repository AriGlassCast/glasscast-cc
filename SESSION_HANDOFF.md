...
| `news-data.json` | Screen 2 | News crawl headlines from the daily PrimeSight brief (refreshed ~6am ET by `refresh-news-crawl`) |
...
- News crawl: curated major general-news headlines, cable-ticker style (static — no API; refresh manually on request). Replaced the old DOOH-industry stats June 12 2026.
- 9 tasks, 6 reminders
...
- News crawl: LIVE — headline-only feed from the daily PrimeSight Morning Brief email (plexus@glasscastmedia.com). Scheduled task `refresh-news-crawl` runs ~6am ET, parses the “Industry & Market News” section, writes `news-data.json` (repo root), single-commit force-push. Screen 2 fetches `news-data.json` every 15 min and repopulates `#newsCrawl` (/// separators, content doubled for seamless loop). Fetch failure or a no-brief morning keeps the last headlines. The daily task does NOT bump version.txt (no needless reloads).
